...
start_server() {
    ...
    npx tsx watch --exclude 'logs/*' --exclude 'data/*' --exclude 'vite.config.ts.timestamp*' src/server/index.ts >> "$PROJECT_ROOT/logs/server.log" 2>&1 &
    SERVER_PID=$!
...
cleanup() {
    ...
    kill_port {{PORT}} "server"
    exit 0
}

trap cleanup INT TERM EXIT

echo "🛑 Stopping any existing {{PROJECT_NAME}} server..."
...
echo ""
echo "📍 Services:"
echo "   • Web UI & API: http://localhost:{{PORT}}"
echo ""
